...
export default function GlobalSettings({
  ...
  }, [visible, loadAllTokens, loadGlobalSettings, checkCLIStatus]);
  ...
  const panel = (
        <>
          ...
          <div className="flex-1 p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
            ...
            {activeTab === 'api-keys' && <AgentApiKeysPanel />}
    ...
    <>
      ...
      {selectedProvider && (
        <ServiceConnectionModal
          key={`service-connection-${selectedProvider}`}
  ...
  if (isPage) {
    return (
      <>
        <div className="flex h-full min-h-[640px] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white">
          {panel}
        </div>
        {extras}
      </>
  ...
  return (
    <AnimatePresence>
      <div key="global-settings-shell" className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        />
        <MotionDiv
          className="relative flex h-[700px] w-full max-w-5xl flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl"
