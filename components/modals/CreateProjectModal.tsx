...
export default function CreateProjectModal({ open, onClose, onCreated, onOpenGlobalSettings }: CreateProjectModalProps) {
  ...
  const connectToProjectWebSocket = (projectId: string) => {
    ...
    const resolveHttpWarmupUrl = () => {
      ...
      throw new Error('Unable to resolve WebSocket warmup URL');
    };

    const connect = () => {
      void (async () => {
        try {
          await fetch(resolveHttpWarmupUrl(), {
            method: 'GET',
            headers: { 'x-ws-warmup': '1' },
          });
        } catch {
          // Warm-up is best-effort; the upgrade handler may already be attached.
        }
        ...
        socket.onerror = () => {
          console.error('❌ Initialization WebSocket error');
        };
      })();
    };
