...
function readDeployError(text: string): string {
  try {
...
export default function ServiceConnectionModal({ 
  ...
  const unwrapToken = (payload: unknown): ServiceToken | null => {
    if (!payload || typeof payload !== 'object') return null;
    const root = payload as Record<string, unknown>;
    const data = (root.data && typeof root.data === 'object' ? root.data : root) as Record<string, unknown>;
    if (typeof data.id !== 'string' || typeof data.provider !== 'string') {
      return null;
    }
    return {
      id: data.id,
  ...
  const loadSavedToken = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        const tokenData = unwrapToken(await response.json());
        setSavedToken(tokenData);
      } else {
        setSavedToken(null);
      }
    } catch (error) {
      console.warn('Failed to load saved token:', error instanceof Error ? error.message : error);
      setSavedToken(null);
    }
  }, [provider]);
  ...
  const handleSaveToken = async () => {
    if (!token.trim()) {
      setFormError('Please enter a valid token');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        const savedTokenData = unwrapToken(await response.json());
        if (savedTokenData) {
        ...
        setFormError(await readErrorMessage(response, 'Failed to save token'));
      }
    } catch (error) {
      console.warn('Failed to save token:', error instanceof Error ? error.message : error);
      setFormError('Could not reach the server. Try saving the token again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteToken = async () => {
    ...
    try {
      ...
      console.warn('Failed to delete token:', error instanceof Error ? error.message : error);
      setFormError('Failed to delete token. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      if (action === 'deploy') {
        ...
        if (response.ok) {
          ...
          const error = await response.text();
          alert(`Failed to deploy: ${readDeployError(error)}`);
        }
      }
    } catch (error) {
