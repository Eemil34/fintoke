...
type CursorErrorKind = 'resource_exhausted' | 'network' | 'auth' | 'invalid_model';
...
async function handleCursorFailure(
  ...
  if (detectedError === 'resource_exhausted') {
    ...
    headerMessage = [
      '⚠️ Cursor Agent rejected the selected model.',
      'Claudable will use Auto (or GPT-5.3 Codex) on the next try. Pick one of those in the model menu.',
    ].join('\n');
  }
...
export async function initializeNextJsProject(
  ...
  const fullPrompt = await buildInitialAgentPrompt(projectId, initialPrompt);
...
function detectCursorError(stderrLines: string[]): CursorErrorKind | undefined {
  ...
  if (lower.includes('unauthorized') || lower.includes('authentication') || lower.includes('api key') || lower.includes('not logged in')) {
    return 'auth';
  }
  if (lower.includes('cannot use this model') || lower.includes('unknown model') || lower.includes('invalid model')) {
...
${buildSiteImageAgentRules(instruction)}
...
${instruction.trim()}`;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (detectedError === 'invalid_model' && attempt < maxAttempts) {
      const modelIndex = baseArgs.indexOf('--model');
      if (modelIndex >= 0) {
        baseArgs[modelIndex + 1] = 'auto';
      } else {
        baseArgs.push('--model', 'auto');
      }
      await persistMessage(
        projectId,
        {
          ...
          messageType: 'chat',
          content:
            'Cursor Agent rejected the selected model. Retrying with Auto.',
          metadata: { cli_type: 'cursor' },
        },
        requestId,
      );
