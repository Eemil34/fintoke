...
export async function markUserRequestAsCompleted(id: string): Promise<void> {
  ...
  void settleSiteImagesForRequest(id);
}

async function settleSiteImagesForRequest(requestId: string): Promise<void> {
  try {
    ...
    const { settleGeneratedSiteImages } = await import('@/lib/templates/siteImages');
    const project = await getProjectById(request.projectId);
    const projectPath =
      project?.repoPath ||
      path.join(process.cwd(), process.env.PROJECTS_DIR || './data/projects', request.projectId);
    await settleGeneratedSiteImages(projectPath);
  } catch (error) {
    console.warn('[UserRequests] Failed to restore image fallbacks:', error);
  }
}
