...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const imageLines = processedImages.map((image, idx) => {
      const filename = path.basename(image.path);
      const sitePath = image.publicUrl || `/uploads/${filename}`;
      return `Image #${idx + 1}: site path ${sitePath} (file ${image.path})`;
    });
    const imageGuide = processedImages.length
      ? `The user uploaded ${processedImages.length} photo(s) into this site's public/uploads folder. Use these exact Next.js src values (for example in <Image src="..." /> or SiteImage). Do not replace them with Unsplash, placeholders, or invented URLs.\n${imageLines.join('\n')}`
      : '';
    const finalInstruction = [instructionWithoutLegacyPaths, imageGuide]
