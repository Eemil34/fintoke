...
const persistProjectPreferences = useCallback(
        ...
        if (vercelConnection && vercelConnection.service_data) {
          ...
          if (url) {
            ...
            setDeploymentStatus((current) =>
              current === 'deploying' || current === 'error' ? current : 'ready',
            );
          } else {
            setDeploymentStatus((current) =>
              current === 'deploying' || current === 'error' ? current : 'idle',
            );
          }
        } else {
          setPublishedUrl(null);
    ...
    deployPollRef.current = setInterval(async () => {
      try {
        ...
        if (isError) {
          ...
          setPublishError('Vercel reported that this deployment failed. Check the Vercel dashboard for build logs.');
        ...
        if (isReady) {
          const rawUrl = data.last_deployment_url || data.deployment_url;
          if (rawUrl) {
            const url = String(rawUrl).startsWith('http') ? rawUrl : `https://${rawUrl}`;
            setPublishedUrl(url);
          }
        ...
        console.warn('Deployment polling failed', error instanceof Error ? error.message : 'unknown error');
      }
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      if (!vercelRes.ok) {
        throw new Error(readApiErrorMessage(responseText, 'Failed to trigger Vercel deployment'));
      }
      ...
      if (data.status === 'READY' && data.deployment_url) {
        const url = data.deployment_url.startsWith('http')
          ? data.deployment_url
          : `https://${data.deployment_url}`;
        setPublishedUrl(url);
        setDeploymentStatus('ready');
        setPublishLoading(false);
      } else if (!data.deployment_id) {
        setDeploymentStatus('error');
        setPublishLoading(false);
        setPublishError('Vercel did not start a deployment.');
      }
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Publish failed. Check GitHub and Vercel settings.';
  ...
  const loadSettings = useCallback(async (projectSettings?: { cli?: string; model?: string }) => {
    try {
      ...
      console.warn('Failed to load settings:', error instanceof Error ? error.message : error);
      const hasCliSet = projectSettings?.cli || preferredCli;
      const hasModelSet = projectSettings?.model || selectedModel;
  ...
  async function handlePreviewImageUpload(event: ChangeEvent<HTMLInputElement>) {
    const files = event.target.files;
    ...
    try {
      ...
      await runAct(
        'Use the photos I just uploaded on this site. Put them in the real pages instead of placeholders.',
        uploaded,
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      try {
        ...
        console.error('Failed to initialize chat view:', error instanceof Error ? error.message : error);
      }
    };
              ...
              <div className="bg-white border-b border-gray-200 px-4 h-[73px] flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    multiple
                    className="hidden"
                    onChange={(event) => void handlePreviewImageUpload(event)}
                  />
                  <button
                    type="button"
                    className="h-9 px-3 flex items-center gap-2 bg-gray-100 text-gray-700 hover:text-gray-900 hover:bg-gray-200 rounded-lg transition-colors text-sm font-medium disabled:opacity-50"
                    onClick={() => previewImageInputRef.current?.click()}
                    disabled={previewUploading || isRunning}
                    title="Upload photos onto this site"
                  >
                    <ImageIcon size={14} />
                      ...
                      <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 p-5">
                        ...
                        {deploymentStatus === 'error' && (
                          <div className="mb-4 p-4 bg-red-50 rounded-lg border border-red-200 ">
                            ...
                            <p className="text-xs text-red-600 ">{publishError || 'There was an error during deployment. Please try again.'}</p>
                          </div>
                        )}
          ...
          <div className="relative w-full max-w-lg bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 ">Upload this site to Vercel. GitHub is saved as a backup.</p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              {deploymentStatus === 'error' && (
                <div className="p-4 rounded-xl border border-red-200 bg-red-50 ">
                  ...
                  <p className="mt-1 text-xs text-red-600 ">{publishError || 'Please try again.'}</p>
                </div>
              )}
