...
export default function HomePage() {
  ...
  const normalizeProjectPayload = useCallback((project: any): ProjectSummary => {
    ...
    return {
      ...
      websiteTemplateId: project.websiteTemplateId ?? project.website_template_id ?? null,
    };
  ...
  const [templatePickedManually, setTemplatePickedManually] = useState(false);
  ...
  const { templates: websiteTemplates } = useTemplates();
  ...
  const suggestedTemplate = suggestWebsiteTemplate(prompt, websiteTemplates);
  const resolvedCloneUrl = parsePublicHttpUrl(cloneUrl);
  ...
  useEffect(() => {
    ...
    if (template) {
      ...
      setTemplatePickedManually(true);
    }
    if (clone) {
      setCloneUrl(clone);
    }
  ...
  useEffect(() => {
    if (templatePickedManually) return;
    setSelectedTemplateId(suggestedTemplate?.id ?? null);
  }, [suggestedTemplate?.id, templatePickedManually]);
  ...
  useEffect(() => {
    ...
    fetchCliStatusSnapshot()
      ...
      .catch((error) => {
        console.error('Failed to check CLI status:', error instanceof Error ? error.message : error);
  ...
  const handleSubmit = async () => {
    const resolvedCloneUrl = parsePublicHttpUrl(cloneUrl);
    if ((!prompt.trim() && uploadedImages.length === 0 && !resolvedCloneUrl) || isCreatingProject) return;
    ...
    const projectName = resolvedCloneUrl
      ? (prompt.trim() ? prompt.slice(0, 50) + (prompt.length > 50 ? '...' : '') : `Clone of ${hostnameFromUrl(resolvedCloneUrl)}`)
      : prompt.slice(0, 50) + (prompt.length > 50 ? '...' : '');
    const initialPrompt = prompt.trim() || (resolvedCloneUrl ? `Recreate ${resolvedCloneUrl} as a reusable website template.` : '');
    ...
    try {
      ...
      const response = await fetchAPI(`${API_BASE}/api/projects`, {
        ...
        body: JSON.stringify({
          ...
          selectedModel,
          websiteTemplateId: resolvedCloneUrl ? undefined : selectedTemplateId || undefined,
      ...
      setTemplatePickedManually(false);
      ...
      <div className="relative z-10 flex h-full w-full">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-2 border-t border-gray-200 ">
            <button
              onClick={() => router.push('/dashboard')}
              className="w-full flex items-center gap-2 p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all text-sm mb-1"
            >
      ...
      <div className="relative flex-1 flex flex-col min-w-0">
        <div className="absolute right-6 top-5 z-20">
          <button
            type="button"
            onClick={() => router.push('/dashboard')}
            className="rounded-full border border-gray-200 bg-white/90 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm backdrop-blur hover:bg-white hover:text-gray-900"
          >
            Dashboard
          </button>
        </div>
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-5xl">
            ...
            >
              ...
              <div className="flex gap-1 flex-wrap items-center">
                ...
                <div className="ml-auto flex items-center gap-1">
                  <button
                    type="submit"
                    disabled={(!prompt.trim() && uploadedImages.length === 0 && !resolvedCloneUrl) || isCreatingProject}
            ...
            <div className="mb-8">
              <TemplatePicker
                selectedId={resolvedCloneUrl ? null : selectedTemplateId}
                suggestedId={resolvedCloneUrl ? null : suggestedTemplate?.id ?? null}
              ...
              <div className="mt-4 rounded-2xl border border-gray-200 bg-white px-4 py-3">
                <label className="block text-sm font-medium text-gray-900">
                  Or clone a live website
                </label>
                <p className="mt-1 text-xs text-gray-500">
                  The agent rebuilds that public site as this project. Save it as a template from chat when it looks right.
                </p>
                <input
                  value={cloneUrl}
                  onChange={(event) => {
                  ...
                  className="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm text-gray-900 outline-none focus:border-gray-400"
                />
              </div>
            </div>
